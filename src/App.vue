...
export default {
  ...
  mounted: function(){
    ...
    let listener = qq.maps.event.addListener(this.map,'click',
      function(event) {
          alert('您点击的位置为:[' + '纬度' + event.latLng.getLat() + ','
                                   + '经度' + event.latLng.getLng() + ']');
      }
    );
    this.addUserMarker(31.305468476254635,121.50784492492676);
    for (let i = 0; i < parkGPSList.length; i++) {
      this.addTargetMarker(parkGPSList[i].lat, parkGPSList[i].lng);
    };
  },
  methods:{
    ...
    getCenter : function(){
      alert("latlng:" + this.map.getCenter());
    },
    ...
    let start_lat = this.map.getCenter().lat;
    let start_lng = this.map.getCenter().lng;
    let stop_lat  = lat;
    let stop_lng  = lng;
